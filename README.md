...
<head>
    ...
    <title>6-Player Dino Battle Royale</title>
...
<body>
    ...
    <div class="panel">
        ...
        <div id="status">Connecting to gaming server...</div>
    ...
    <div class="chat-container">
        <div id="chat-log"><i>System: Connect or type a room code to play with up to 6 players!</i></div>
        <div class="chat-row">
    ...
    <script>
        ...
        // Player custom generation cosmetics
        ...
        // GAME VARIABLES
        let p1 = { id: myShortId, x: 50, y: 110, vy: 0, isJumping: false, isDead: false, color: myColor, score: 0 }; 
        let players = {}; // FIXED: Holds up to 5 external players dynamically
        let cactus = { x: 600, y: 115, width: 15, height: 20, speed: 5 };
        
        let lastScoreTick = Date.now();

        // Connect to central network matrix
        mqttClient = mqtt.connect('wss://broker.hivemq.com:8884/mqtt');
        
        mqttClient.on('connect', () => {
            document.getElementById('status').innerText = "✅ Online! Create a Room Code or Find a Match.";
        ...
        function joinRoomCode() {
            ...
            if (!targetRoom) return alert("Please type a room code!");
            setupRoomChannels(targetRoom);
        }

        function findRandomMatch() {
            setupRoomChannels("dino_battle_royale_lobby_2026");
        }

        function setupRoomChannels(roomName) {
            currentRoom = roomName;
            mqttClient.subscribe('dino/' + currentRoom);
            
            // UI adjustment
            document.getElementById('connection-controls').style.display = 'none';
            document.getElementById('randomBtn').style.display = 'none';
            document.getElementById('leaveBtn').style.display = 'inline-block';
            document.getElementById('status').innerText = `🎮 Connected to Room: ${roomName}`;
            appendLog(`<i>System: Entered room ${roomName}. Max limit 6 players.</i>`);

            // Listen for data streams
            mqttClient.on('message', (topic, msg) => {
                const data = JSON.parse(msg.toString());
                if (data.id === myShortId) return; // Skip ourselves
                ...
                if (data.type === 'leave') {
                    ...
                    appendLog(`<i>System: Player [${data.id.substring(0,4)}] left the match.</i>`);
                    return;
                }

                // FIXED: Dynamically map and bound new incoming configurations up to 6 players max
                if (Object.keys(players).length < 5 || players[data.id]) {
                    if (!players[data.id]) {
                        appendLog(`<i>System: Player [${data.id.substring(0,4)}] entered the battle!</i>`);
                    }
                    
                    players[data.id] = {
                        x: 50 + (Object.keys(players).indexOf(data.id) !== -1 ? Object.keys(players).indexOf(data.id) + 1 : Object.keys(players).length + 1) * 35,
                        y: data.y,
                        isDead: data.isDead,
                        score: data.score,
                        color: data.color
                    ...
                    // Sync speed/cactus parameters if guest matches host payload
                    if (isHost()) {
                        if (data.speedSync) {
        ...
        // Determine Room Master Host (Lowest numerical ID active inside room matrix)
        function isHost() {
        ...
        function sendChat() {
            ...
            if (currentRoom) {
                mqttClient.publish('dino/' + currentRoom, JSON.stringify({ type: 'chat', id: myShortId, color: myColor, text: text }));
                appendLog(`<b style="color: ${myColor};">You:</b> ${escapeHTML(text)}`);
                input.value = '';
            } else {
                alert("Connect to a match or room first!");
            }
        }

        function leaveMatch() {
            if (currentRoom) {
                mqttClient.publish('dino/' + currentRoom, JSON.stringify({ type: 'leave', id: myShortId }));
                mqttClient.unsubscribe('dino/' + currentRoom);
            }
            currentRoom = null;
            players = {};
            ...
            document.getElementById('status').innerText = "✅ Online! Create a Room Code or Find a Match.";
            appendLog("<i>System: You left the match.</i>");
        }
        ...
        function loop() {
            ...
            // Cactus management via Master Host
            if (isHost()) {
            ...
            // Continuous packet broadcast to the MQTT room channel
            if (currentRoom) {
                mqttClient.publish('dino/' + currentRoom, JSON.stringify({
                    type: 'update',
                    id: myShortId,
                    y: p1.y,
                    isDead: p1.isDead,
