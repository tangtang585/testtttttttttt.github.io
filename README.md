...
<head>
    ...
    <title>6-Player Dino (Position Bug Fixed)</title>
...
<body>
    ...
    <h1>🦖 6-Player Dino (Position Bug Fixed)</h1>
    
    <div class="panel">
        <div>Your Room Code: <strong id="my-id" style="color:#0070f3; font-size: 1.2rem;">...</strong></div>
        
        <div id="lobby-count">👥 Players in lobby: 1 / 6</div>
    ...
    <script>
        ...
        try {
            if (typeof mqtt !== 'undefined') {
                ...
                mqttClient.on('connect', () => {
                    ...
                    document.getElementById('chat-log').innerHTML = "<i>System: Connected to server network layer. Ready!</i>";
                });
                ...
                mqttClient.on('message', (topic, msg) => {
                    ...
                    try {
                        ...
                        if (data.type === 'update') {
                            ...
                            // FIXED POSITIONING LOGIC: Sort all IDs so locations never overlap or glitch out
                            let activeKeys = Object.keys(players);
                            if (!players[data.id]) activeKeys.push(data.id);
                            activeKeys.push(myShortId);
                            activeKeys.sort();

                            // Adjust our own X layout dynamic positioning slot
        ...
        function updateLobbyCount() {
            ...
            document.getElementById('lobby-count').innerText = `👥 Players in lobby: ${total} / 6`;
        }

        function joinRoomCode() {
            const targetRoom = document.getElementById('friend-code').value.trim();
            if (!targetRoom) return alert("Please type a room code first!");
            setupRoomChannels(targetRoom);
        }

        function findRandomMatch() {
            setupRoomChannels("dino_battle_royale_global_lobby");
        }

        function setupRoomChannels(roomName) {
            if (!isServerConnected) return alert("Still connecting to network. Try again in 2 seconds.");
            if(currentRoom) mqttClient.unsubscribe('dino_fixed/' + currentRoom);
            
            currentRoom = roomName;
            players = {}; 
            mqttClient.subscribe('dino_fixed/' + currentRoom);
            ...
            appendLog(`<i>System: Entered room code [${roomName}].</i>`);
            updateLobbyCount();
        }
        ...
        function sendChat() {
            ...
            if (currentRoom && isServerConnected) {
                mqttClient.publish('dino_fixed/' + currentRoom, JSON.stringify({ type: 'chat', id: myShortId, color: myColor, text: text }));
        ...
        function leaveMatch() {
            if (currentRoom && isServerConnected) {
                mqttClient.publish('dino_fixed/' + currentRoom, JSON.stringify({ type: 'leave', id: myShortId }));
                mqttClient.unsubscribe('dino_fixed/' + currentRoom);
            }
            currentRoom = null;
            players = {};
            p1.x = 40;
        ...
        function loop() {
            ...
            if (currentRoom && isServerConnected) {
                mqttClient.publish('dino_fixed/' + currentRoom, JSON.stringify({
                    type: 'update',
                    id: myShortId,
                    y: p1.y,
                    isDead: p1.isDead,
